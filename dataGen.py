...
class DataGenerator:
	...
	def generate_hm(self, img_name = None, img_dir = None, direction = 0, size = 64, pts=None, out_name = None, out_dir = None):
		...
		if img_name != None:
			...
			if pts is None:
				if direction != -1:
					pts=self.get_center_coord(direction,version=2) #TODO version
				else:
			...
			for i in range(2):
				if not(np.array_equal(pts[i], [-1,-1])):
					...
					im[:,:,i] = self._gaussian(size, size, pts[i][0],pts[i][1],sigma= 10)
				else:
					im[:,:,i] = np.zeros((height,width))
		# return hm
			# im = np.zeros(shape=(size,size,3))

			# im[:,:,0] = self.gaussian(size, size, pts[0][0], pts[0][1], 3) # left
			# im[:,:,1] = self.gaussian(size, size, pts[1][0], pts[1][1], 3) # right
			# if out_name != None:
			# 	# im = im*255 #TODO
			# 	im = im.astype('float32')
			# 	cv2.imwrite(out_name,cv2.cvtColor(im, cv2.COLOR_BGR2RGB)) 
		# all img in a dir
		elif img_dir != None:
			count = 0
			path_imgs = img_dir
	...
	def get_center_coord(self,direction,version=0, deltx=-1, delty=-1):
		if version==0: # eye center point based on true pts coord
			deltx = 128 - 64
			delty = 150 - 64
			coord = [[[0,0],[0,0]],[[90, 123],[156, 123]],[[95, 125],[160, 123]],[[100,125],[165, 123]],[[88, 129],[152, 127]],[[96, 129],[160, 127]],[[104, 129],[168, 127]],[[92, 131],[157, 129]],[[95, 131],[160, 129]],[[98, 131],[163, 129]]]
