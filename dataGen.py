...
class DataGenerator:
	...
	def _gaussian(self, height,width, c1_x, c1_y, sigma = 1):
		...
		return np.exp(-((x-c1_x)**2 + (y-c1_y)**2) / 2*sigma**2)
	...
	def crop_im(self,image,points=None,size=(64,64)):
		if points is None:
			points = np.array([127., 124.], np.int32) 
		else:
			XC = points[27][0] # No.28 facial points
			YC = points[27][1]
   
		leftx = int(round(max(XC-size[0],0)))
		rightx = int(round(min(XC,image.shape[1])))

		miny = int(round(max(YC-size[1]//2,0)))
		maxy = int(round(min(YC+size[1]//2,image.shape[0])))
		image_l = image[miny:maxy,leftx:rightx,: ]

		leftx = int(round(max(XC,0)))
		rightx = int(round(min(XC+size[0],image.shape[1])))
	...
	# ======================= Test set generator ==================
	def _create_test_table(self,gt_txt=None):
		self.test_table = []
		self.test_dict = {}
		input_file = open(gt_txt, 'r')
		for line in input_file:
			if line in ['\n', '\r\n']:
				print('READING end of file')
				break
			...
			eyes = list(map(int,line[3:]))
			eyes = np.reshape(eyes, (-1,2))
			self.test_dict[name] = {'gtMap' : gtMap, 'direction' : direction, 'eyes' : eyes}
			self.test_table.append(name)
		input_file.close()
		print('Test table created')

	def _test_set_generator(self,batch_path,num_image=None):
		if num_image is None:
			num_image = len(self.test_table)
		test_img = np.zeros((num_image, 64,64,3), dtype = np.float32)
		test_gt_pts = np.zeros((num_image,2,2), np.float32)
		test_direction = np.zeros(num_image, np.float32)
		for i in range(num_image):
			name = self.test_table[i]
			img = cv2.imread(batch_path+'/'+name)
			if img is None:
				print('err file ',name) 
			img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
			test_img[i] = img.astype(np.float32) 
			test_gt_pts[i] = self.test_dict[name]['eyes']
			test_direction[i] = self.test_dict[name]['direction']
		print('set generation done.')
		return test_img,test_gt_pts,test_direction

	# ======================= Update Dataset =======================

	def add_train_table(self,name_list,gtMap_list,dir_num,pts):
		origin_num=len(self.train_table)
		for i in range(len(name_list)):
			name = name_list[i]
			gtMap = gtMap_list[i]
			w = [1] * 1
			eyes=np.zeros((2,2), np.int32)
			if len(pts) == 2:
				eyes[0] = pts
				eyes[1] = pts
			else:
			...
			self.data_dict[name] = {'gtMap' : gtMap, 'direction' : dir_num[i], 'eyes' : pts[i], 'weights' : w}
			self.train_table.append(name)
		print('Original training images number: ',origin_num, ' Updated dataset amount: ', len(self.train_table))
		...
		with open(filename, 'a') as file:
			if len(direction)>1:
				for i in range(len(direction)):
					line=img_path[i]+' '+gt_path[i]+' '+str(direction[i])+' '+str(pts[i][0])+' '+str(pts[i][1])+' '+str(pts[i][0])+' '+str(pts[i][1])+'\n'
					print('writing: '+line)
					file.write(line)
			else:
	...
	def get_center_coord(self,direction,version=0, deltx=-1, delty=-1):
		if version==0: # eye center point based on true pts coord
			coord = [[[0,0],[0,0]],[[26,32],[27,28]],[[36,25],[34,32]],[[41,30],[43,25]],[[20,33],[24,30]],[[38,39],[34,34]],[[44,34],[45,30]],[[25,42],[30,35]],[[34,38],[35,36]],[[38,40],[40,35]]]
