...
def open_img(name, flag, color = 'RGB'):
    ...
    if flag == 0: # img
        ...
        filename = os.path.join('train_x/left', name)
    elif flag == 2: # gtMap right
        filename = os.path.join('train_x/right', name)
    elif flag == 3: # gtMap right
        filename = os.path.join('gt_img/left', name)
    elif flag == 4: # gtMap right
        filename = os.path.join('gt_img/right', name)
...
## generate heatmap/rotate/flip/colour perturbation for all files in  XX txt(need to specify txt filename)
count = 0
