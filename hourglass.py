...
class HourglassModel():
    def __init__(self, nFeat = 64, nStack = 4, nLow = 4, outputDim = 1, batch_size = 16, attention = False, training = True, drop_rate = 0.2, lear_rate = 2.5e-4, decay = 0.96, decay_step = 2000, dataset = None, w_summary = True, logdir_train = None, logdir_test = None,model_dir = None, tiny = True, w_loss = False, modif = True, name = 'tiny_hourglass', gpu=False):
        """ Initializer
        Args:
    ...
    def pred_direction(self, img, version=1):
        ''' 64*64*2 img
        '''
    ...
    def _train(self, nEpochs=3, epochiter=2, saveStep=2, validIter=10):
        with tf.name_scope('Train'):
            ...
            for epoch in range(nEpochs):
                ...
                if tempAccuracy< (np.sum(accuracy_array) / len(accuracy_array)):
                    tempAccuracy = np.sum(accuracy_array) / len(accuracy_array)
                else:
                    print('Stop training')
                    break
