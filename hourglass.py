...
class HourglassModel():
	...
	def _compute_err(self, u, v, pts=None):
		...
		# if pts is None:
		# 	v_y_l,v_x_l,v_y_r,v_x_r = self._argmax(v)
		# else:
		# 	v_x_l = pts[0][0]
		# 	v_y_l = pts[0][1]
		# 	v_x_r = pts[1][0]
		# 	v_y_r = pts[1][1]
		# left_err = tf.divide(tf.sqrt(tf.square(tf.to_float(u_x_l - v_x_l)) + tf.square(tf.to_float(u_y_l - v_y_l))), tf.to_float(72))
		# right_err = tf.divide(tf.sqrt(tf.square(tf.to_float(u_x_r - v_x_r)) + tf.square(tf.to_float(u_y_r - v_y_r))), tf.to_float(72))
	...
	def confidence(self, img, pts, eye):
		...
		# u_y_l,u_x_l,u_y_r,u_x_r = self._argmax(img)
		# 11 = sqrt(5*5+10*10)
		# left_err = tf.divide(tf.sqrt(tf.square(tf.to_float(u_x_l - pts[0][0])) + tf.square(tf.to_float(u_y_l - pts[0][1]))), tf.to_float(11))
		# right_err = tf.divide(tf.sqrt(tf.square(tf.to_float(u_x_r - pts[1][0])) + tf.square(tf.to_float(u_y_r - pts[1][1]))), tf.to_float(11))
		# l_confidence = tf.subtract(tf.to_float(1), left_err)
		# r_confidence = tf.subtract(tf.to_float(1), right_err)
		# return (l_confidence,r_confidence)

	def pred_direction(self, img, version=2):
		''' 64*64*2 img
		'''
		tmp = [[0,0],[0,0]] # store max confidence [0][] left max [1][] right max
		other = 0 # the other side confidence
		direction = [0,0]
		total_l = total_r = 0
		for i in range(10):
