...
# x_path_imgs = '/Dataset/train_x'
# y_path_imgs = '/Dataset/train_y'
...
class HourglassModel():
	def __init__(self, nFeat = 256, nStack = 4, nLow = 4, outputDim = 3, batch_size = 16, attention = False, training = True, drop_rate = 0.2, lear_rate = 2.5e-4, decay = 0.96, decay_step = 2000, dataset = None, w_summary = True, logdir_train = None, logdir_test = None,model_dir = None, tiny = True, w_loss = False, modif = True, name = 'tiny_hourglass'):
		""" Initializer
		Args:
	...
	def generate_model(self):
		...
		with tf.name_scope('model'):
			self.output = self.fullNetwork(self.x, 256, 4, 3)
			print('--Graph : Done')
	...
	def conv2d(self, inputs, filters, kernel_size = 1, strides = 1, pad = 'VALID', name = None):
		...
		# kernel = tf.get_variable('weight', shape=[kernel_size,kernel_size, inputs.get_shape().as_list()[3], filters], initializer=tf.random_normal_initializer())
			# print('shape = ', inputs.get_shape().as_list())
	...
	def fullNetwork(self, inputs, nFeat = 256, nStack = 4, outDim = 3):
		with tf.name_scope('model'):
			...
			print("full newtwork: temp output shape = " + str(tmp_out.shape))
	...
	def _compute_err(self, u, v):
		...
		"""
		u_x,u_y = self._argmax(u) ##TODO what it does?
	...
	def _accuracy_computation(self):
		...
		"""
		# self.imgAccur = [] #outDim
		for i in range(len(self.direction)): ## TODO joints=direction
			...
			for epoch in range(nEpochs):
				...
				with tf.name_scope('epoch_' + str(epoch)):
					for i in range(epochiter):
						x_batch, y_batch, w_batch = next(self.generator)
	...
	def addTrain():
		print ('add data to train set')
		# according to confidence add to training set